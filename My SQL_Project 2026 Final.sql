...
-- c) Show the invoice_id, client_id, invoice_total, payment_total, invoice_date and due_date from the ‘invoices’ 
...
-- b) -- Find clients without any invoices, output their names (with details)
SELECT 
    c.client_id,
    c.client_name,
    c.street_address,
...
-- d) Partition and rank clients based on their invoice_total from invoices table
SELECT 
    client_id,
    invoice_id,
    invoice_total,
